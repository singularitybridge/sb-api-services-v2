...
const API_BASE_URL = process.env.ENDPOINT_URL
  ? process.env.ENDPOINT_URL.replace('/assistant/user-input', '')
  : 'http://localhost:3000';
const SESSION_TOKEN = process.env.SESSION_TOKEN;

if (!SESSION_TOKEN) {
  throw new Error(
    'SESSION_TOKEN environment variable is not set. Please set it in your .env file for e2e tests.',
  );
}
...
describe('Session Management API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to create initial session: ${response.status} ${errorBody}`,
      );
    }
  ...
  test('POST /session/clear should end current session and return a new one', async () => {
    ...
    const getNewSessionResponse = await fetch(
      `${API_BASE_URL}/session/${newSessionData._id}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${SESSION_TOKEN}`,
    ...
    const getOldSessionResponse = await fetch(
      `${API_BASE_URL}/session/${initialSessionId}`,
      {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${SESSION_TOKEN}`,
