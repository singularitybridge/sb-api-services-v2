...
describe('Memory API Endpoints', () => {
  let createdEntryId;
  const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NmQ0Y2MzMjg0NjEyMjMzNDEzYmViNzciLCJlbWFpbCI6ImF2aUBzaW5ndWxhcml0eWJyaWRnZS5uZXQiLCJjb21wYW55SWQiOiI2NmQ0MWFjMzQ4N2MxOWY2ZDRjMjNmYTEiLCJpYXQiOjE3NDgyMTI4NTAsImV4cCI6MTc0ODgxNzY1MH0.v5j_7aXMLrf_Y-8O1_HFVEw-UY1REp_xVN5naeo6CWY';
  ...
  test('GET /api/memory/entries - should retrieve journal entries', async () => {
    if (!createdEntryId) {
      throw new Error("Skipping GET /entries test as createdEntryId is undefined. POST test likely failed or did not set ID.");
    }
    const response = await request(app)
      .get(`/api/memory/entries?tags=supertest`) // Removed userId and companyId from query
      .set('Authorization', `Bearer ${token}`);
    if (response.statusCode === 400) {
      console.log('GET /api/memory/entries validation errors:', JSON.stringify(response.body.errors, null, 2));
    }
    expect(response.statusCode).toBe(200);
    expect(Array.isArray(response.body)).toBe(true);
    const foundEntry = response.body.find(entry => entry._id === createdEntryId);
  ...
  test('GET /api/memory/entries/search - should search journal entries', async () => {
    if (!createdEntryId) {
      throw new Error("Skipping GET /entries/search test as createdEntryId is undefined.");
    }
    // Mock runVectorSearch to return the created entry's ID
    const { runVectorSearch } = require('../../src/services/vector.search.service');
    ...
    const response = await request(app)
      ...
      .set('Authorization', `Bearer ${token}`);
    if (response.statusCode === 400) {
      console.log('GET /api/memory/entries/search validation errors:', JSON.stringify(response.body.errors, null, 2));
    }
    expect(response.statusCode).toBe(200);
    expect(Array.isArray(response.body)).toBe(true);
    const foundEntry = response.body.find(entry => entry._id === createdEntryId);
  ...
  test('GET /api/memory/entries/friendly - should retrieve friendly journal entries', async () => {
    if (!createdEntryId) {
      throw new Error("Skipping GET /entries/friendly test as createdEntryId is undefined.");
    }
    const response = await request(app)
      .get(`/api/memory/entries/friendly?tags=supertest`) // Removed userId and companyId from query
      .set('Authorization', `Bearer ${token}`);
    if (response.statusCode === 400) {
      console.log('GET /api/memory/entries/friendly validation errors:', JSON.stringify(response.body.errors, null, 2));
    }
    expect(response.statusCode).toBe(200);
    expect(Array.isArray(response.body)).toBe(true);
    const foundEntry = response.body.find(entry => entry._id === createdEntryId);
  ...
  test('PATCH /api/memory/entries/:id - should update a journal entry', async () => {
    if (!createdEntryId) {
      throw new Error("Skipping PATCH /entries/:id test as createdEntryId is undefined.");
    }
    const response = await request(app)
    ...
    expect(response.body.tags).toEqual(expect.arrayContaining(['supertest', 'updated']));
  ...
  test('DELETE /api/memory/entries/:id - should delete a journal entry', async () => {
    if (!createdEntryId) {
      throw new Error("Skipping DELETE /entries/:id test as createdEntryId is undefined.");
    }
    const response = await request(app)
  ...
  test('GET /api/memory/entries - should not find deleted entry', async () => {
    ...
    const response = await request(app)
      ...
      .set('Authorization', `Bearer ${token}`);
    if (response.statusCode === 400) {
      console.log('GET /api/memory/entries (after delete) validation errors:', JSON.stringify(response.body.errors, null, 2));
    }
    expect(response.statusCode).toBe(200);
    const foundEntry = response.body.find(entry => entry._id === createdEntryId);
