...
describe('Session ID Flow in Action Execution', () => {
  ...
  it('should use current session ID in tool execution, not cached ID', async () => {
    ...
    console.log('1. Tool is created with initial context (may have old session ID)');
    ...
    console.log('   - Session.findById should be called with: ' + currentSessionId);
    console.log('   - executeFunctionCall should receive: ' + currentSessionId);
    console.log('4. Action messages should publish to channel: sb-' + currentSessionId);
