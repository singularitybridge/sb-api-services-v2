  ...
  attachments: z
    .array(
      z.object({
        ...
        data: z.string().describe('The attachment data (URL or base64 string)'),
        mimeType: z
          .string()
          .optional()
...
export async function executeAssistant(
  ...
  try {
    const result = await apiClient.executeAssistant({
      assistantId: input.assistantId,
      userInput: input.userInput,
      sessionId: input.sessionId,
      systemPromptOverride: input.systemPromptOverride,
      attachments: input.attachments as
        | Array<{
            type: 'url' | 'base64';
            data: string;
            mimeType?: string;
...
export const executeAssistantTool = {
  ...
  description:
    "Execute an AI assistant with a prompt. Returns the assistant's response including any tool calls, thinking, or generated content.",
