...
export const createReplicateActions = (
  context: ActionContext,
): FunctionFactory => ({
  runReplicateModel: {
    description:
      'Run any model on Replicate. Input must be a JSON string. Handles image outputs by uploading to storage.',
    strict: true,
    parameters: {
      type: 'object',
      properties: {
        model: {
          type: 'string',
          description:
            'The Replicate model identifier (e.g., "owner/model-name" or "owner/model-name:version-id").',
        },
        inputJsonString: {
          // Changed from 'input'
          type: 'string',
          description:
            'A JSON string representing the input object for the specified model.',
        },
        outputType: {
          type: 'string',
          enum: ['image', 'text', 'json'],
          description:
            'Optional: Specify the expected output type to help guide processing. Defaults to "image" if output is a URL.',
        },
        filename: {
          type: 'string',
          description:
            'Optional: If the output is an image, this will be used as the filename when uploading to storage.',
        },
      },
    ...
    function: async (args: ReplicateModelWithStringInputArgs) => {
      ...
      if (typeof model !== 'string' || model.trim().length === 0) {
        throw new Error(
          'Invalid model identifier: Must be a non-empty string.',
        );
      }
      try {
        ...
        throw new Error(
          'Invalid inputJsonString: Must be a valid JSON string.',
        );
      }
      if (typeof inputObject !== 'object' || inputObject === null) {
        // This case should ideally be caught by JSON.parse, but as a safeguard
        throw new Error(
          'Parsed inputJsonString did not result in a valid object.',
        );
      }

      try {
        const result = await runReplicateModel(context.companyId, {
          model,
          input: inputObject,
          filename,
        });
  ...
  runReplicateImageModel: {
    description:
      'Run an image-generating or image-processing model on Replicate. Input must be a JSON string. The image output will be uploaded to storage.',
    strict: true,
    parameters: {
      type: 'object',
      properties: {
        ...
        inputJsonString: {
          ...
          type: 'string',
          description:
            'A JSON string representing the input object for the image model.',
        },
        filename: {
          type: 'string',
          description:
            'Optional: Custom filename for the generated/processed image when uploading to storage.',
        },
      },
      required: ['model', 'inputJsonString'], // Changed from 'input'
      additionalProperties: false,
    ...
    function: async (args: ReplicateImageModelWithStringInputArgs) => {
      ...
      try {
        ...
        const imageUrl = await runReplicateModel(context.companyId, {
          model,
          input: inputObject,
          filename,
        });
        ...
        console.error(
          'runReplicateImageModel: Error running image model',
          error.message,
        );
        throw error;
      }
