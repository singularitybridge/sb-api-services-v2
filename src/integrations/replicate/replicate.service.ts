...
export const createPrediction = async (
  ...
  try {
    ...
    console.log(
      `[ReplicateService] Prediction created successfully. ID: ${response.data.id}`,
    );
    return response.data;
  } catch (error: any) {
    console.error(
      '[ReplicateService] Error creating Replicate prediction:',
...
export const getPredictionStatus = async (
  ...
  try {
    ...
    console.error(
      `[ReplicateService] Error getting Replicate prediction status for ID ${predictionId}:`,
      error.response?.data || error.message,
    );
    throw new Error(
      `Failed to get Replicate prediction status: ${
        error.response?.data?.detail || error.message
      }`,
    );
  }
...
export const waitForPrediction = async (
  ...
  console.log(
    `[ReplicateService] Starting to wait for prediction ID: ${predictionId}. Max wait: ${POLLING_CONFIG.maxWaitTime}ms`,
  );

  while (
    status !== 'succeeded' &&
    status !== 'failed' &&
    elapsedTime < POLLING_CONFIG.maxWaitTime
  ) {
    console.log(
      `[ReplicateService] Polling for ${predictionId}. Elapsed: ${elapsedTime}ms. Interval: ${interval}ms. Status: ${
        status || 'initial'
      }`,
    );
    ...
    console.log(
      `[ReplicateService] Polled ${predictionId}. Current status: ${status}`,
    );
    ...
    if (status === 'succeeded') {
      ...
      console.error(
        `[ReplicateService] Prediction ${predictionId} failed. Error: ${error}`,
      );
      throw new Error(
        `Replicate prediction failed: ${error || 'Unknown error'}`,
      );
    }
  ...
  if (status !== 'succeeded') {
    console.warn(
      `[ReplicateService] Prediction ${predictionId} timed out or did not succeed after ${elapsedTime}ms. Final status: ${status}`,
    );
...
export const downloadAndUploadImage = async (
  ...
  try {
    ...
    if (filename) {
      ...
      const ext =
        nameParts.length > 1 ? nameParts.pop()?.toLowerCase() : undefined;
    ...
    if (
      ...
      if (typeParts[0] === 'image' && typeParts[1]) {
        ...
        if (determinedContentType !== 'image/png')
          determinedContentType = 'image/png'; // Correct if inconsistent
      }
    }

    if (!finalFileName) {
      finalFileName = filename
        ? filename.endsWith('.' + determinedExtension)
          ? filename
          : `${filename}.${determinedExtension}`
    ...
    if (
      finalFileName.endsWith('.octet-stream') &&
      determinedExtension !== 'octet-stream'
    ) {
    ...
    console.log(
      `[ReplicateService] Uploading to GCS. OriginalName: ${file.originalname}, MimeType: ${file.mimetype}`,
    );
