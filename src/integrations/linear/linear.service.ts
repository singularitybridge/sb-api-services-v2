...
export const fetchIssues = async (
  ...
  try {
    ...
    if (!issues || !issues.nodes) {
      ...
      return {
        success: false,
        error: 'Unexpected response structure from Linear API',
      };
    }
...
export const createIssue = async (
  ...
  description: string,
  teamId: string,
): Promise<{ success: boolean; data?: IssuePayload; error?: string }> => {
  try {
    const linearClient = await createLinearClient(companyId);
...
export const updateIssue = async (
  ...
  try {
    ...
    if (updateData.status) {
      ...
      const status = states.nodes.find(
        (s) => s.name.toLowerCase() === updateData.status!.toLowerCase(),
      );
      if (!status) {
...
export const fetchAllIssues = async (
  ...
  try {
    ...
    while (hasNextPage) {
      const result: IssueConnection = await linearClient.issues({
        first: 100,
        after: endCursor,
      });
...
export const fetchIssuesByDate = async (
  companyId: string,
  days: number,
): Promise<{ success: boolean; data?: Issue[]; error?: string }> => {
  try {
    ...
    const result = await linearClient.issues({
      filter: {
        or: [{ createdAt: { gt: date } }, { updatedAt: { gt: date } }],
      },
    });
    return { success: true, data: result.nodes };
  } catch (error) {
...
export const fetchIssueStatuses = async (
  companyId: string,
): Promise<{ success: boolean; data?: WorkflowState[]; error?: string }> => {
  try {
    const linearClient = await createLinearClient(companyId);
...
export const createComment = async (
  ...
  issueId: string,
  body: string,
): Promise<{ success: boolean; data?: CommentPayload; error?: string }> => {
  try {
    const linearClient = await createLinearClient(companyId);
