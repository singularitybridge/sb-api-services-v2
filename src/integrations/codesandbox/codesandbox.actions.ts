  ...
  return {
    createSandbox: {
      ...
      function: async (
        ...
        const result = await createSandboxService(
          context.companyId,
          templateId,
        );
        if (result.success) {
        ...
        try {
          ...
          if (result.success) {
            ...
            const errorDetails = result.details
              ...
              : '';
            throw new Error(
              `${result.error || 'Command execution failed'}${errorDetails}`,
            );
          }
        } catch (error) {
    ...
    readFileFromSandbox: {
      ...
      function: async (
        ...
        throw new Error(
          result.error || 'Failed to read file from CodeSandbox sandbox',
        );
      },
    },
    writeFileToSandbox: {
      ...
      function: async (
        ...
        const result = await writeFileToSandbox(
          context.companyId,
          params.sandboxId,
          params.path,
          params.content,
        ...
        throw new Error(
          result.error || 'Failed to write file to CodeSandbox sandbox',
        );
      },
    },
    listSandboxes: {
      description:
        'Lists all sandboxes for the authenticated user with their public URLs.',
      parameters: {
        type: 'object',
        properties: {
          limit: {
            type: 'number',
            description:
              'Maximum number of sandboxes to return. Default is 20.',
          },
        },
        required: [],
      },
    ...
    getSandboxInfo: {
      description:
        'Gets detailed information about a specific sandbox including all available URLs.',
      parameters: {
        type: 'object',
        properties: {
          sandboxId: {
      ...
      function: async (
        ...
        const result = await getSandboxInfo(
          context.companyId,
          params.sandboxId,
        );
        if (result.success) {
    ...
    getSandboxUrl: {
      description:
        'Gets various URLs for accessing a sandbox (public, embed, standalone, etc.).',
      parameters: {
        type: 'object',
        properties: {
          sandboxId: {
