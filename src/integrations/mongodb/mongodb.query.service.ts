...
const executeQuery = async (queryParams: QueryParams): Promise<any> => {
  try {
    logger.log('Executing query with params:', JSON.stringify(queryParams, null, 2));
...
const parseUserInput = (input: string): QueryParams => {
  ...
  if (!match) {
    throw new Error('Invalid query format. Expected format: db.collection.operation(filter, update/projection).limit().skip().sort()');
  }
  ...
  if (filterStr) {
    try {
      ...
      throw new Error(`Invalid filter format: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  if (updateOrProjectionStr) {
    try {
      ...
      throw new Error(`Invalid update/projection format: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  if (sortValue) {
    try {
      ...
      throw new Error(`Invalid sort format: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  if (!['find', 'updateOne', 'updateMany'].includes(operation)) {
    throw new Error(`Unsupported operation: ${operation}. Supported operations are: find, updateOne, updateMany`);
  }
...
const handleUserQuery = async (input: string): Promise<any> => {
  try {
    ...
    throw new Error(`Failed to handle user query: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
};
