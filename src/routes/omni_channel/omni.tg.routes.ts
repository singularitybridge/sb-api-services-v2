...
if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
  ...
  console.warn(
    'Twilio credentials (TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN) not found. Some omni.tg routes may not be available or fully functional.',
  );
...
messagingRouter.post('/whatsapp/reply', async (req, res) => {
  ...
  if (!twilioClient || !twilioPhoneNumber) {
    console.error(
      'Twilio client or phone number not initialized. Cannot send reply via Twilio.',
    );
