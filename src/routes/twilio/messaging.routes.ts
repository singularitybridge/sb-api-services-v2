...
if (process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN) {
  ...
  twilioMessagingRouter.post('/whatsapp/reply', async (req, res) => {
    ...
    if (typeof response === 'string') {
      ...
      if (!twilioClient || !twilioPhoneNumber) {
        console.error(
          'Twilio client or phone number not initialized. Cannot send WhatsApp reply.',
        );
        return res
          .status(503)
          .send('Twilio client or phone number not initialized');
      }
      ...
      console.error(
        'handleSessionMessage did not return a string for Twilio WhatsApp reply.',
      );
  ...
  console.warn(
    'Twilio credentials (TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN) not found. Twilio messaging routes will not be available.',
  );
  // Optionally, define placeholder routes or a message indicating the service is unavailable
  twilioMessagingRouter.use((req, res) => {
    res
      .status(503)
      .send('Twilio messaging service is not configured on the server.');
  });
}
