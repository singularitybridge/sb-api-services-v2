...
export const executeHandler = async (req: AuthenticatedRequest, res: any) => {
    ...
    try {
      ...
      if (!req.user || !req.company) {
        return res.status(401).json({ error: 'User or company context not found. Authentication may have failed.' });
      }

      // Validate assistant exists and belongs to company
      console.log(`[Execute Route] Looking for assistant with ID: ${assistantId}`);
      ...
      if (!assistant) {
        ...
        if (assistantExists) {
          console.log(`[Execute Route] Assistant exists but belongs to company: ${assistantExists.companyId}`);
        } else {
          console.log(`[Execute Route] Assistant with ID ${assistantId} does not exist`);
        }
        return res.status(404).json({ error: 'Assistant not found or access denied for this company.' });
      }
      ...
      if (clientWantsSSE) {
        ...
        if (
          ...
          console.error('executeAssistantStateless did not return a valid textStream for SSE');
          if (!res.writableEnded) {
            res.write(
              `event:error\ndata:${JSON.stringify({type:'error', errorDetails:{message: 'Failed to establish stream'}})}\n\n`
            );
          }
        }
        ...
        if (responseFormat?.type === 'json_object' || responseFormat?.type === 'json_schema') {
          ...
          res.status(500).json({ error: 'Internal server error: Unexpected response format' });
        }
      }
    } catch (error) {
      ...
      if (clientWantsSSE && !res.writableEnded) { 
        res.write(`event:error\ndata:${JSON.stringify({type:'error', errorDetails:{message: err.message || 'An unknown error occurred'}})}\n\n`);
        res.end();
