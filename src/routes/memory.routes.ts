...
const GetEntriesQuerySchema = z.object({
  sessionId: z.string().optional().refine((val) => !val || mongoose.Types.ObjectId.isValid(val), { message: "Invalid sessionId" }),
  ...
  userId: z.string().optional().refine((val) => !val || mongoose.Types.ObjectId.isValid(val), { message: "Invalid userId" }), // Made optional
  companyId: z.string().optional().refine((val) => !val || mongoose.Types.ObjectId.isValid(val), { message: "Invalid companyId" }), // Made optional
});
...
const SearchEntriesQuerySchema = z.object({
  ...
  userId: z.string().optional().refine((val) => !val || mongoose.Types.ObjectId.isValid(val), { message: "Invalid userId" }), // Already optional
  companyId: z.string().optional().refine((val) => !val || mongoose.Types.ObjectId.isValid(val), { message: "Invalid companyId" }), // Made optional
});
...
// POST /entries
router.post('/entries', validate(CreateEntrySchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!userId || !companyId) {
      ...
      return res.status(401).json({ message: "User or Company ID missing from authenticated session." });
    }
    ...
    const entry = await createJournalEntryService(journalData, apiKey, ChannelType.WEB); // Changed ChannelType.API to ChannelType.WEB
...
// GET /entries
router.get('/entries', validate(GetEntriesQuerySchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    let { userId, companyId, sessionId, entryType, tags, limit, scope } = req.query as any;
    ...
    const entries = await getJournalEntriesService(userId, companyId, sessionId, entryType, tags, limit, scope);
...
// GET /entries/search
router.get('/entries/search', validate(SearchEntriesQuerySchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!q) { // q is required by schema, but good to double check
        return res.status(400).json({ message: "Search query 'q' is required." });
    }

    const entries = await searchJournalEntriesService(q, companyId, userId, entryType, tags, limit);
    res.status(200).json(entries);
  } catch (error) {
    next(error);
  }
...
router.get('/entries/friendly', validate(GetEntriesQuerySchema), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    let { userId, companyId, sessionId, entryType, tags, limit, scope } = req.query as any;
    ...
    const entries = await getFriendlyJournalEntriesService(userId, companyId, sessionId, entryType, tags, limit, scope);
    res.status(200).json(entries);
  } catch (error) {
    next(error);
  }
...
// PATCH /entries/:id
router.patch('/entries/:id', validate(UpdateEntrySchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
