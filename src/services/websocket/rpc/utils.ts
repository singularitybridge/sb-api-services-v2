...
const createJsonRpcRequest = (method: string, params?: any): JsonRpcRequest => ({
  jsonrpc: '2.0',
  method,
  params,
  id: uuidv4()
...
const createJsonRpcErrorResponse = (id: string, code: JsonRpcErrorCode, message: string): JsonRpcResponse => ({
  jsonrpc: '2.0',
  error: { code, message },
  id
});

const createJsonRpcSuccessResponse = (id: string, result: any): JsonRpcResponse => ({
  jsonrpc: '2.0',
  result,
  id
});

export const registerRpcMethod = (method: string, handler: RpcHandler): void => {
...
export const callRpcMethod = async (
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(new Error(`RPC call to ${method} timed out after ${options.timeout || DEFAULT_TIMEOUT}ms`));
...
export const handleRpcRequest = async (
  ...
  try {
    ...
    socket.emit('message', JSON.stringify(
      createJsonRpcSuccessResponse(request.id, result)
    ));
  } catch (error) {
    socket.emit('message', JSON.stringify(
      createJsonRpcErrorResponse(
