...
export const sendMessage = (socket: Socket, message: JsonRpcErrorResponse | string): void => {
  socket.emit('message', typeof message === 'string' ? message : JSON.stringify(message));
};

export const handleMessage = async (socket: AuthenticatedSocket, message: any): Promise<void> => {
