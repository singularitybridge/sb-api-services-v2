...
export const validateContentData = async (
  ...
  if (!contentType) {
    return {
      isValid: false,
      errors: [{ field: 'contentTypeId', message: 'ContentType not found' }],
      contentType: null,
    };
  }
  ...
  if (!data || typeof data !== 'object') {
    return {
      isValid: false,
      errors: [
        { field: 'data', message: 'Content data is missing or invalid' },
      ],
      contentType,
    };
  }
  ...
  contentType.fields.forEach((fieldDef) => {
    ...
    if (fieldDef.required && (value === undefined || value === null)) {
      errors.push({
        field: fieldDef.name,
        message: `Field '${fieldDef.name}' is required.`,
      });
      return;
    }
    ...
    if (value !== undefined && value !== null) {
      ...
      if (validator && !validator(value)) {
        errors.push({
          field: fieldDef.name,
          message: `Field '${fieldDef.name}' must be a ${fieldType}.`,
        });
      }

      if (
        fieldDef.enum &&
        fieldDef.enum.length > 0 &&
        ...
        errors.push({
          ...
          message: `Field '${
            fieldDef.name
          }' must be one of [${fieldDef.enum.join(', ')}].`,
        });
      }
    }
  });
