...
export const updateFile = async (
  ...
  try {
    const headers = await getHeaders(companyId);
    const response = await axios.put(`${BASE_URL}/b/${binId}`, data, {
      headers,
    });
...
export const updateArrayElement = async (
  companyId: string,
  binId: string,
  arrayKey: string,
  elementId: string,
  ...
  try {
    ...
    const elementIndex = currentData[arrayKey].findIndex(
      (item: any) => item.id === elementId,
    );

    if (elementIndex === -1) {
      throw new Error(
        `Element with id '${elementId}' not found in the array '${arrayKey}'`,
      );
    }

    if (useMerge) {
      currentData[arrayKey][elementIndex] = mergeDeep(
        currentData[arrayKey][elementIndex],
        updateData,
      );
    } else {
      currentData[arrayKey][elementIndex] = {
    ...
    const response = await axios.put(`${BASE_URL}/b/${binId}`, currentData, {
      headers,
    });
    return response.data.record;
  } catch (error) {
...
export const deleteArrayElement = async (
  ...
  try {
    ...
    currentData[arrayKey] = currentData[arrayKey].filter(
      (item: any) => item.id !== elementId,
    );
...
export const insertArrayElement = async (
  companyId: string,
  binId: string,
  arrayKey: string,
  newElement: any,
...
export const cloneJsonbin = async (
  companyId: string,
  binId: string,
): Promise<string> => {
  try {
