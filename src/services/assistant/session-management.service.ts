...
const transformMessageToOpenAIFormat = (mongoMessage: IMessage) => {
  ...
  if (mongoMessage.sender === 'system' && (textualContent === undefined || textualContent.trim() === '')) {
...
export async function getSessionMessages(apiKey: string, sessionId: string) {
  ...
  const mongoMessages = await Message.find({ sessionId: new mongoose.Types.ObjectId(sessionId) })
  ...
  const formattedMessages = await Promise.all(
    mongoMessages.map(async (msg) => {
      const openAIFormattedMessage = transformMessageToOpenAIFormat(msg as IMessage);
